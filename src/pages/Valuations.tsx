...
// --- GRAHAM ---
const Graham = () => {
  const [p, setP] = useState({ ticker: 'BBSE3', price: 35.06, lpa: 3.57, vpa: 8.43 });
  const fv = Math.sqrt(22.5 * p.lpa * p.vpa);
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card>
        <CardHeader><CardTitle className="text-base">Premissas — Graham</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          <FieldRow label="Ticker" value={p.ticker} onChange={v => setP({ ...p, ticker: v })} type="text" />
          ...
          <Button className="w-full gap-2 mt-2"><Save className="h-4 w-4" /> Salvar</Button>
        </CardContent>
      </Card>
...
// --- BAZIN ---
const Bazin = () => {
  const [p, setP] = useState({ ticker: 'BBSE3', price: 35.06, avgDiv: 2.67, minDY: 6 });
...
// --- BUFFETT ---
const Buffett = () => {
  const [p, setP] = useState({ ticker: 'WIZC3', price: 9.32, roe: 24, payout: 50, lpa: 2.33, years: 10, pl: 15 });
...
// --- LYNCH ---
const Lynch = () => {
  const [p, setP] = useState({ ticker: 'BBSE3', price: 35.06, pl: 9.82, growth: 23.22 });
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-lg bg-muted">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider">PEG Ratio</p>
              <p className={`text-lg font-bold font-mono mt-1 ${peg < 1 ? 'text-positive' : 'text-negative'}`}>{peg.toFixed(2)}</p>
            </div>
            <div className="p-3 rounded-lg bg-muted">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Veredicto</p>
              <p className="text-lg font-bold mt-1">{peg < 1 ? '✅ Barato' : peg < 1.5 ? '⚠️ Justo' : '❌ Caro'}</p>
            </div>
            <div className="p-3 rounded-lg bg-muted">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Preço Justo (PEG=1)</p>
              <p className="text-lg font-bold font-mono mt-1 text-primary">{formatBRL(fv)}</p>
            </div>
            <div className="p-3 rounded-lg bg-muted">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Preço Máx. Compra</p>
              <p className="text-lg font-bold font-mono mt-1">{formatBRL(fv * 0.75)}</p>
            </div>
          </div>
...
const VFF = ({ years }: { years: 3 | 5 }) => {
  const initialProfits = years === 3
    ? [875924000, 954757180, 1040685304]
    : [875924000, 954757180, 1040685304, 1134346981, 1236438129];

  const [p, setP] = useState({
    ticker: 'AZZA3', price: 26.55, shares: 206489000,
    discount: 15, perpetuity: 3, profits: initialProfits,
  });
